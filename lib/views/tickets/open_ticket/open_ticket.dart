...
class _OpenTicketsScreenState extends State<OpenTicketsScreen> with RouteAware {
  ...
  List<Map<String, dynamic>> _getFilteredTickets(List<Map<String, dynamic>> tickets) {
    ...
    return tickets.where((ticket) {
      return ticket['ticketID'].toString().toLowerCase().contains(_searchQuery) ||
          ticket['plazaID'].toString().toLowerCase().contains(_searchQuery) ||
          ticket['vehicleNumber'].toString().toLowerCase().contains(_searchQuery) ||
          ticket['vehicleType'].toString().toLowerCase().contains(_searchQuery) ||
      ...
      child: InkWell(
        ...
        onTap: () {
          ...
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => ChangeNotifierProvider<OpenTicketViewModel>(
                create: (_) => OpenTicketViewModel(),
                child: ModifyViewOpenTicketScreen(ticketId: ticket['ticketID'].toString()),
              ),
            ),
          ).then((_) => _refreshData());
        },
