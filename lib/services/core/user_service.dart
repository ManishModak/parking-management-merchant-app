...
class UserService {
  ...
  Future<Map<String, dynamic>> userRegister({
    ...
    developer.log('[USER] Starting registration at URL: $url', name: 'UserService');
    ...
    try {
      ...
      if (response.statusCode == 201) {
        if (isAppUserRegister) {
          if (responseData['token'] == null || responseData['user']?['id'] == null) {
            throw ServiceException('Invalid response format: missing token or user ID');
          }
          await _storage.storeAuthDetails(
            responseData['token'],
            responseData['user']['id'].toString(),
          );
          developer.log('[USER] Successfully stored auth details for app user', name: 'UserService');
        }
        developer.log('[USER] Successfully registered user', name: 'UserService');
      ...
      developer.log('[USER] Error in userRegister: $e', name: 'UserService', error: e, stackTrace: stackTrace);
      rethrow;
    }
  }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['success'] == true && responseData['user'] != null) {
          ...
          developer.log('[USER] Successfully fetched user info for ID: $userId', name: 'UserService');
      ...
      developer.log('[USER] Error in fetchUserInfo: $e', name: 'UserService', error: e, stackTrace: stackTrace);
      rethrow;
    }
  }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['success'] == true) {
          developer.log('[USER] Successfully updated user info for ID: $userId', name: 'UserService');
          return true;
        }
        return false;
      }
      ...
      developer.log('[USER] Error in updateUserInfo: $e', name: 'UserService', error: e, stackTrace: stackTrace);
      rethrow;
    }
  }
  ...
  Future<List<User>> getUserList() async {
    ...
    try {
      ...
      developer.log('[USER] Error in getUserList: $e', name: 'UserService', error: e, stackTrace: stackTrace);
      rethrow;
    }
  }
  ...
  Future<bool> resetPassword(String userId, String newPassword) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        final responseData = json.decode(response.body);
        if (responseData['success'] == true) {
          developer.log('[USER] Successfully reset password for user ID: $userId', name: 'UserService');
          return true;
        }
        return false;
      }
      ...
      developer.log('[USER] Error in resetPassword: $e', name: 'UserService', error: e, stackTrace: stackTrace);
      rethrow;
    }
  }
  ...
  Future<List<User>> getUsersByEntityId(String entityId) async {
    final url = ApiConfig.getFullUrl('${AuthApi.getUsersByEntity}$entityId');
    final serverUrl = Uri.parse(url);
    ...
    try {
      ...
      if (response.statusCode == 200 && responseData['success'] == true) {
        final List<dynamic> users = responseData['users'] ?? [];
        final userList = users.map((user) => User.fromJson(user)).toList();
        developer.log('[USER] Successfully fetched ${userList.length} users for entity $entityId', name: 'UserService');
        return userList;
      }

      throw _handleErrorResponse(response, 'Failed to fetch users by entity ID');
    } on SocketException catch (e) {
      throw ServerConnectionException('Failed to connect to the auth server: $e');
    } on TimeoutException {
      throw RequestTimeoutException('Request timed out');
    } catch (e, stackTrace) {
      developer.log('[USER] Error in getUsersByEntityId: $e', name: 'UserService', error: e, stackTrace: stackTrace);
      rethrow;
    }
  }
  ...
  Future<String?> getAuthToken() async {
    developer.log('[USER] Retrieving auth token from storage', name: 'UserService');
    final token = await _storage.getAuthToken();
    developer.log('[USER] Auth token ${token != null ? 'retrieved' : 'not found'}', name: 'UserService');
    return token;
  }
  ...
  Future<String?> getUserId() async {
    ...
    developer.log('[USER] User ID ${userId != null ? 'retrieved' : 'not found'}', name: 'UserService');
    return userId;
  }
