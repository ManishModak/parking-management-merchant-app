...
class LaneDetailsViewModel extends ChangeNotifier {
  ...
  LaneDetailsViewModel() {
    developer.log(
        '[LaneDetailsViewModel] Initialized. isEditable: $_isEditable, isFirstTime: $_isFirstTime',
        name: 'LaneDetailsViewModel');
  }

  void toggleEditable() {
    if (_isFirstTime) {
      developer.log(
          '[LaneDetailsViewModel] Cannot toggle editable: isFirstTime is true.',
          name: 'LaneDetailsViewModel.toggleEditable');
      return;
    }
    if (_isLoading) {
      developer.log(
          '[LaneDetailsViewModel] Cannot toggle editable: isLoading is true.',
          name: 'LaneDetailsViewModel.toggleEditable');
      return;
    }
    _isEditable = !_isEditable;
    developer.log(
        '[LaneDetailsViewModel] Toggled editable state to: $_isEditable',
        name: 'LaneDetailsViewModel.toggleEditable');
    notifyListeners();
  }
  ...
  void resetToEditableState() {
    ...
    _isFirstTime = true;
    errors.clear();
    developer.log('[LaneDetailsViewModel] Reset to initial editable state.',
        name: 'LaneDetailsViewModel.resetToEditableState');
  ...
  void clearFieldsAndNotify() {
    developer.log(
        '[LaneDetailsViewModel] Clearing ALL lane data and resetting state.',
        name: 'LaneDetailsViewModel.clearFieldsAndNotify',
        level: 1000);
  ...
  void populateForModification(List<Lane> initialLanes) {
    developer.log(
        '[LaneDetailsViewModel] Populating for modification with ${initialLanes.length} saved lanes.',
        name: 'LaneDetailsViewModel.populateForModification');
    savedLanes.clear();
    ...
    _isLoading = false;
    developer.log(
        '[LaneDetailsViewModel] Population complete. isEditable: $_isEditable, isFirstTime: $_isFirstTime',
        name: 'LaneDetailsViewModel.populateForModification');
  ...
  void clearError(String key) {
    if (errors.containsKey(key)) {
      developer.log('[LaneDetailsViewModel] Clearing error for key: $key',
          name: 'LaneDetailsViewModel.clearError');
      errors.remove(key);
      ...
      if (!errors.keys.any((k) => k != 'general' && errors[k] != null)) {
        ...
        developer.log(
            '[LaneDetailsViewModel] Cleared general error as no specific errors remain.',
            name: 'LaneDetailsViewModel.clearError');
      }
      notifyListeners();
    }
  ...
  void _setLoading(bool value) {
    if (_isLoading != value) {
      _isLoading = value;
      developer.log('[LaneDetailsViewModel] isLoading set to: $_isLoading',
          name: 'LaneDetailsViewModel');
  ...
  void addNewLaneToList(Lane lane) {
    developer.log(
        '[LaneDetailsViewModel] Adding validated NEW lane to list: ${lane.laneName}',
        name: 'LaneDetailsViewModel.addNewLaneToList');
    ...
    developer.log(
        '[LaneDetailsViewModel] Added NEW lane: ${lane.laneName}. New count: ${newlyAddedLanes.length}. Notifying listeners...',
        name: 'LaneDetailsViewModel.addNewLaneToList');
    notifyListeners();
  }
  ...
  void modifyNewLaneInList(int indexInNewList, Lane updatedLane) {
    developer.log(
        '[LaneDetailsViewModel] Modifying NEW lane in list at index $indexInNewList: ${updatedLane.laneName}',
        name: 'LaneDetailsViewModel.modifyNewLaneInList');
    ...
    if (indexInNewList < 0 || indexInNewList >= newlyAddedLanes.length) {
      developer.log(
          '[LaneDetailsViewModel] Error modifying new lane: Invalid index $indexInNewList',
          name: 'LaneDetailsViewModel.modifyNewLaneInList',
          level: 1000);
    ...
    developer.log(
        '[LaneDetailsViewModel] Modified NEW lane at index $indexInNewList to: ${updatedLane.laneName}. Notifying listeners...',
        name: 'LaneDetailsViewModel.modifyNewLaneInList');
    notifyListeners();
  }
  ...
  void removeNewLaneFromList(int indexInNewList) {
    if (indexInNewList >= 0 && indexInNewList < newlyAddedLanes.length) {
      ...
      developer.log(
          '[LaneDetailsViewModel] Removed NEW lane at index $indexInNewList: $removedLaneName. New count: ${newlyAddedLanes.length}. Notifying listeners...',
          name: 'LaneDetailsViewModel.removeNewLaneFromList');
      errors.clear(); // Clear errors after successful removal
      notifyListeners();
    } else {
      developer.log(
          '[LaneDetailsViewModel] Error removing new lane: Invalid index $indexInNewList',
          name: 'LaneDetailsViewModel.removeNewLaneFromList',
          level: 900);
    }
  }
  ...
  Future<List<Lane>> fetchLanes(BuildContext context, String plazaId) async {
    if (_isLoading) {
      developer.log("[LaneDetailsViewModel] Fetch ignored: Already loading.",
          name: "LaneDetailsViewModel.fetchLanes");
      return savedLanes; // Return current list if loading
    }
    _setLoading(true);
    errors.clear();
    developer.log(
        "[LaneDetailsViewModel] Attempting to fetch lanes for Plaza ID: $plazaId",
        name: "LaneDetailsViewModel.fetchLanes");
    try {
      ...
      developer.log(
          "[LaneDetailsViewModel] Fetched ${savedLanes.length} lanes successfully.",
          name: "LaneDetailsViewModel.fetchLanes");
      ...
      developer.log('[LaneDetailsViewModel] UNEXPECTED Error fetching lanes',
          error: e,
  ...
  Future<bool> saveNewlyAddedLanes(BuildContext context, String plazaId) async {
    developer.log(
        '[LaneDetailsViewModel] Attempting saveNewlyAddedLanes: Count=${newlyAddedLanes.length}, isEditable=$_isEditable, isFirstTime=$_isFirstTime',
        name: 'LaneDetailsViewModel.saveNewlyAddedLanes');

    if (!_isEditable && !_isFirstTime) {
      // Allow save if first time, even if not 'editable'
      developer.log(
          '[LaneDetailsViewModel] Save prevented: Step is not editable and not first time.',
          name: 'LaneDetailsViewModel.saveNewlyAddedLanes');
    ...
    if (newlyAddedLanes.isEmpty) {
      developer.log(
          '[LaneDetailsViewModel] No new lanes to save via API. Marking step as complete and locking.',
          name: 'LaneDetailsViewModel.saveNewlyAddedLanes');
    ...
    try {
      ...
      developer.log(
          '[LaneDetailsViewModel] Invalid Plaza ID ($plazaId) for saving lanes.',
          name: 'LaneDetailsViewModel.saveNewlyAddedLanes',
          level: 1000);
    ...
    for (int i = 0; i < newlyAddedLanes.length; i++) {
      ...
      if (validationError != null) {
        ...
        String errorMsg =
            "Lane '${laneToValidate.laneName}' (Index $i): $validationError";
        validationErrorMessages.add(errorMsg);
        developer.log(
            '[LaneDetailsViewModel] Validation failed for lane "${laneToValidate.laneName}": $errorMsg',
            name: 'LaneDetailsViewModel.saveNewlyAddedLanes');
      } else {
    ...
    if (hasValidationErrors) {
      ...
      errors['general'] =
          "Please correct errors in New Lanes:\n- ${validationErrorMessages.join('\n- ')}";
      developer.log(
          '[LaneDetailsViewModel] Validation failed for one or more new lanes.',
          name: 'LaneDetailsViewModel.saveNewlyAddedLanes');
      _setLoading(false);
      return false;
    }
    ...
    final duplicateCheckErrors =
        _checkForDuplicatesAgainstAll(context, validatedLanesToSend);
    if (duplicateCheckErrors.isNotEmpty) {
      errors['general'] =
          "Duplicate entries found (check names/IDs):\n- ${duplicateCheckErrors.join('\n- ')}";
      developer.log(
          '[LaneDetailsViewModel] Duplicate check failed before API call.',
          name: 'LaneDetailsViewModel.saveNewlyAddedLanes');
      _setLoading(false);
      return false;
    }
    developer.log(
        '[LaneDetailsViewModel] Pre-save validation and duplicate checks passed.',
        name: 'LaneDetailsViewModel.saveNewlyAddedLanes');

    // 3. API Call with validated list
    try {
      developer.log(
          '[LaneDetailsViewModel] Calling _laneService.addLane API for ${validatedLanesToSend.length} validated lanes...',
          name: 'LaneDetailsViewModel.saveNewlyAddedLanes');
      List<Lane> createdLanesFromApi = await _laneService
          .addLane(validatedLanesToSend); // Pass validated list
      developer.log(
          '[LaneDetailsViewModel] addLane API successful. Received ${createdLanesFromApi.length} lanes back.',
          name: 'LaneDetailsViewModel.saveNewlyAddedLanes');

      // 4. Update State
      savedLanes.addAll(
          createdLanesFromApi); // Add newly saved lanes to the saved list
      ...
      developer.log('[LaneDetailsViewModel] UNEXPECTED Error saving new lanes',
          error: e,
          stackTrace: stackTrace,
          name: 'LaneDetailsViewModel.saveNewlyAddedLanes',
          level: 1200);
  ...
  Future<bool> updateSavedLane(int indexInSavedList, Lane updatedLaneFromDialog,
      BuildContext context) async {
    developer.log(
        '[LaneDetailsViewModel] Attempting updateSavedLane at index $indexInSavedList for Lane Name: ${updatedLaneFromDialog.laneName}',
        name: 'LaneDetailsViewModel.updateSavedLane');

    // --- Initial Checks ---
    if (!_isEditable) {
      developer.log(
          "[LaneDetailsViewModel] Cannot update saved lane: Step not editable.",
          name: "LaneDetailsViewModel.updateSavedLane");
    ...
    if (indexInSavedList < 0 || indexInSavedList >= savedLanes.length) {
      developer.log(
          "[LaneDetailsViewModel] Cannot update saved lane: Invalid index $indexInSavedList.",
          name: "LaneDetailsViewModel.updateSavedLane",
          level: 1000);
    ...
    if (originalLane.laneId == null || originalLane.laneId! <= 0) {
      developer.log(
          "[LaneDetailsViewModel] Cannot update saved lane: Original lane data missing valid ID.",
          name: "LaneDetailsViewModel.updateSavedLane",
          level: 1000);
      throw PlazaException(S.of(context).errorInvalidLaneData);
    }
    // Validate the ORIGINAL record status needed for update
    if (originalLane.recordStatus == null ||
        !Lane.validRecordStatuses
            .contains(originalLane.recordStatus?.toLowerCase())) {
      developer.log(
          "[LaneDetailsViewModel] Cannot update saved lane: Original lane data missing valid RecordStatus.",
          name: "LaneDetailsViewModel.updateSavedLane",
          level: 1000);
    ...
    developer.log(
        '[LaneDetailsViewModel] Constructed laneToSend: ${laneToSend.toJsonForUpdate()}',
        name: 'LVM.updateSavedLane');

    // --- Validate using Model Method ---
    final String? validationError =
        laneToSend.validateForUpdate(); // USE Model validator

    if (validationError != null) {
      errors['update_validation'] = validationError; // Use direct error message
      developer.log(
          "[LaneDetailsViewModel] Validation failed for update: $validationError",
          name: "LaneDetailsViewModel.updateSavedLane");
      _setLoading(false);
      throw PlazaException(S.of(context).validationFailed,
          serverMessage: validationError); // Pass validation error back
    }
    developer.log(
        '[LaneDetailsViewModel] Model validation successful for update.',
        name: 'LVM.updateSavedLane');

    // --- Duplicate Check ---
    final List<Lane> otherLanesToCheck = [
      ...savedLanes
          .where((l) => l.laneId != originalLane.laneId), // Exclude self
      ...newlyAddedLanes
    ];
    final Map<String, String> duplicateErrors =
        _checkForDuplicates(laneToSend, otherLanesToCheck);
    if (duplicateErrors.isNotEmpty) {
      ...
      developer.log(
          "[LaneDetailsViewModel] Duplicate check failed for update: $errorMsg",
          name: "LaneDetailsViewModel.updateSavedLane");
      _setLoading(false);
      throw PlazaException(S.of(context).validationDuplicateGeneral,
          serverMessage: errorMsg); // Pass duplicate error back
    }
    developer.log(
        '[LaneDetailsViewModel] Duplicate check successful for update.',
        name: 'LVM.updateSavedLane');

    // --- API Call ---
    try {
      developer.log(
          '[LaneDetailsViewModel] Calling _laneService.updateLane API for lane ID: ${laneToSend.laneId}...',
          name: 'LaneDetailsViewModel.updateSavedLane');
      bool success = await _laneService
          .updateLane(laneToSend); // Pass the validated object
      if (success) {
        savedLanes[indexInSavedList] = laneToSend; // Update local list
        errors.clear();
        developer.log(
            "[LaneDetailsViewModel] Successfully Updated SAVED lane ID: ${laneToSend.laneId}. Notifying.",
            name: "LaneDetailsViewModel.updateSavedLane");
        _setLoading(false);
        notifyListeners();
        return true; // Indicate success
        ...
        developer.log(
            "[LaneDetailsViewModel] updateLane API returned false for ID: ${laneToSend.laneId}.",
            name: "LaneDetailsViewModel.updateSavedLane",
            level: 900);
        throw ServiceException(
            S.of(context).messageErrorUpdatingLaneServer); // Specific message
      }
    } on PlazaException catch (e) {
      // Catch specific exceptions first
      ...
      developer.log(
          '[LaneDetailsViewModel] UNEXPECTED Error updating saved lane',
          error: e,
          stackTrace: stackTrace,
          name: 'LaneDetailsViewModel.updateSavedLane',
          level: 1200);
  ...
  Map<String, String> _checkForDuplicates(
      Lane laneToCheck, List<Lane> againstLanes) {
    final duplicateErrors = <String, String>{};
    developer.log(
        '[LaneDetailsViewModel] Checking duplicates for lane "${laneToCheck.laneName}" (ID: ${laneToCheck.laneId}) against ${againstLanes.length} other lanes.',
    ...
    for (var existingLane in againstLanes) {
      ...
      if (laneToCheck.laneId != null &&
          existingLane.laneId != null &&
          laneToCheck.laneId == existingLane.laneId) {
      ...
      if (isDuplicateValue(laneToCheck.laneName, existingLane.laneName)) {
        ...
        developer.log(
            '[LaneDetailsViewModel] Duplicate LaneName found: ${laneToCheck.laneName}',
            name: '_checkForDuplicates');
      }
      if (isDuplicateValue(
          laneToCheck.plazaId.toString() + laneToCheck.plazaLaneId.toString(),
          existingLane.plazaId.toString() +
              existingLane.plazaLaneId.toString())) {
        duplicateErrors['plazaLaneId'] = "Duplicate Plaza Lane ID";
        developer.log(
            '[LaneDetailsViewModel] Duplicate PlazaLaneID found: ${laneToCheck.plazaLaneId} for Plaza ${laneToCheck.plazaId}',
            name: '_checkForDuplicates');
      }
      if (isDuplicateValue(
          laneToCheck.rfidReaderId, existingLane.rfidReaderId)) {
        duplicateErrors['RFIDReaderID'] = "Duplicate RFID ID";
        developer.log(
            '[LaneDetailsViewModel] Duplicate RFIDReaderID found: ${laneToCheck.rfidReaderId}',
            name: '_checkForDuplicates');
      }
      if (isDuplicateValue(laneToCheck.cameraId, existingLane.cameraId)) {
        duplicateErrors['CameraID'] = "Duplicate Camera ID";
        developer.log(
            '[LaneDetailsViewModel] Duplicate CameraID found: ${laneToCheck.cameraId}',
            name: '_checkForDuplicates');
      }
      if (isDuplicateValue(laneToCheck.wimId, existingLane.wimId)) {
        duplicateErrors['WIMID'] = "Duplicate WIM ID";
        developer.log(
            '[LaneDetailsViewModel] Duplicate WIMID found: ${laneToCheck.wimId}',
            name: '_checkForDuplicates');
      }
      if (isDuplicateValue(
          laneToCheck.boomerBarrierId, existingLane.boomerBarrierId)) {
        duplicateErrors['BoomerBarrierID'] = "Duplicate Boomer Barrier ID";
        developer.log(
            '[LaneDetailsViewModel] Duplicate BoomerBarrierID found: ${laneToCheck.boomerBarrierId}',
            name: '_checkForDuplicates');
      }
      if (isDuplicateValue(laneToCheck.ledScreenId, existingLane.ledScreenId)) {
        duplicateErrors['LEDScreenID'] = "Duplicate LED Screen ID";
        developer.log(
            '[LaneDetailsViewModel] Duplicate LEDScreenID found: ${laneToCheck.ledScreenId}',
            name: '_checkForDuplicates');
      }
      if (isDuplicateValue(
          laneToCheck.magneticLoopId, existingLane.magneticLoopId)) {
        duplicateErrors['MagneticLoopID'] = "Duplicate Magnetic Loop ID";
        developer.log(
            '[LaneDetailsViewModel] Duplicate MagneticLoopID found: ${laneToCheck.magneticLoopId}',
            name: '_checkForDuplicates');
      }
    }
    developer.log(
        '[LaneDetailsViewModel] Duplicate check result for "${laneToCheck.laneName}": Found ${duplicateErrors.length} duplicates.',
        name: '_checkForDuplicates');
    return duplicateErrors;
  }

  // Checks for duplicates across all lanes (saved + candidates) - useful before batch save
  List<String> _checkForDuplicatesAgainstAll(
      BuildContext context, List<Lane> candidateLanes) {
    developer.log(
        '[LaneDetailsViewModel] Checking for duplicates against ALL (${savedLanes.length} saved) for ${candidateLanes.length} candidates.',
        name: '_checkForDuplicatesAgainstAll');
    final List<Lane> combinedPotentialLanes = [
      ...savedLanes,
      ...candidateLanes
    ];
    ...
    void checkAndAddDuplicate(
        Set<String> seenSet, String? value, String fieldNameForError,
        {String? mapKey}) {
      if (value == null || value.trim().isEmpty) return;
      final key = mapKey ??
          value
              .trim()
              .toLowerCase(); // Use mapKey if provided (for combined IDs)
      if (seenSet.contains(key)) {
        // Only add the error message once per unique duplicate value
        String errorMsg = S
            .of(context)
            .validationDuplicate("$fieldNameForError '${value.trim()}'");
        if (!duplicateErrorMessages.contains(errorMsg)) {
          duplicateErrorMessages.add(errorMsg);
          developer.log(
              '[LaneDetailsViewModel] Duplicate found for $fieldNameForError: $value',
              name: '_checkForDuplicatesAgainstAll');
        }
      } else {
        seenSet.add(key);
    ...
    for (final lane in combinedPotentialLanes) {
      ...
      checkAndAddDuplicate(
          seenPlazaLaneIds, lane.plazaLaneId.toString(), "Plaza Lane ID",
          mapKey: "${lane.plazaId}_${lane.plazaLaneId}");
      ...
      checkAndAddDuplicate(
          seenBoomerIds, lane.boomerBarrierId, "Boomer Barrier ID");
      checkAndAddDuplicate(seenLedIds, lane.ledScreenId, "LED Screen ID");
      checkAndAddDuplicate(
          seenLoopIds, lane.magneticLoopId, "Magnetic Loop ID");
    }

    developer.log(
        '[LaneDetailsViewModel] Duplicate check against all found ${duplicateErrorMessages.length} issues.',
        name: '_checkForDuplicatesAgainstAll');
  ...
  void _handleServiceError(
    ...
    if (e is HttpException) {
      ...
      // Assuming PlazaException has message/serverMessage/statusCode
      errorMessage = e.serverMessage ?? e.message ?? defaultMessage;
      statusCode = e.statusCode;
      ...
      errorMessage =
          S.of(context).errorUnexpected; // Fallback for other Exceptions
    }
    developer.log(
        '[LaneDetailsViewModel] Handling Service Error: ${e.runtimeType} - "$errorMessage" ${statusCode != null ? '(Status: $statusCode)' : ''}',
        error: e,
        stackTrace: StackTrace.current,
        name: 'LaneDetailsViewModel._handleServiceError',
        level: 900);
  ...
  void _handleGenericError(BuildContext context, dynamic e) {
    ...
    developer.log('[LaneDetailsViewModel] Handling Generic Error: $e',
        error: e,
        stackTrace: StackTrace.current,
        name: 'LaneDetailsViewModel._handleGenericError',
        level: 1000);
    if (context.mounted) {
  ...
  @override
  void dispose() {
    developer.log('[LaneDetailsViewModel] Disposing...',
        name: 'LaneDetailsViewModel');
    super.dispose();
    developer.log('[LaneDetailsViewModel] Dispose complete.',
        name: 'LaneDetailsViewModel');
  }
}
